    ...
    >
      ...
      <div className="hidden md:flex items-center gap-1">
        {navLinks.map((link) => (
          <a
            ...
            className={`px-3 py-1.5 font-bold uppercase text-[10px] tracking-[2.5px] transition-colors duration-[200ms] hover:opacity-70 ${
              scrolled ? 'text-[#3F261F]' : 'text-[#E8E6E1]'
            }`}
          >
            {link.label}
      ...
      <div className="hidden md:flex items-center gap-2 ml-2">
        <a
          ...
          className="inline-flex items-center px-5 py-2 border border-transparent bg-[#5D6D59] text-[#E8E6E1] font-bold uppercase text-[10px] tracking-[2px] rounded-full relative overflow-hidden group transition-transform duration-[200ms] hover:scale-[1.02]"
          style={{ transitionTimingFunction: 'cubic-bezier(0.25, 0.1, 0.25, 1)' }}
        >
          <span
        ...
        <div className="absolute top-full left-0 right-0 mt-2 bg-[#DDDAD3]/95 backdrop-blur-[16px] rounded-[20px] border border-[#C8C4BC]/40 p-4 md:hidden shadow-[0_8px_32px_rgba(22,25,16,0.12)]">
          {navLinks.map((link) => (
            <a
              ...
              href={link.href}
              onClick={scrollTo(link.href)}
              className="block py-2.5 font-bold uppercase text-[11px] tracking-[2.5px] text-[#3F261F]"
            >
              {link.label}
            </a>
          ))}
