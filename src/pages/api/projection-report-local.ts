// LOCAL COPY of api/projection-report.ts (Fede's Vercel edge fn), re-exposed as
// an Astro endpoint so it runs under `astro dev`. Fede's original is untouched.
// Inner AirROI logic is identical; only the request wrapper + env read are adapted.
